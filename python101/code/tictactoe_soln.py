...
def make_client(args, password='BadKangaroo'):
...
def move_and_win(client, game_id):
    ...
    If so, modify game state and upload it.
    """
    state = client.get('tictactoe', game_id)
    if state['move'] != 'X':
        print 'Not your move'
        return state
    ...
    state['move'] = 'game over'

    # TODO: ideally this should be a PUT, not a POST
    client.post('tictactoe', game_id, state)
    return state
